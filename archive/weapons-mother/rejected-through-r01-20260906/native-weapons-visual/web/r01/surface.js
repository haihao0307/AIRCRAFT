...
function createSurfaces(T){
 ...
 function apply(v){if(!v||Object.keys(v).length!==4)throw Error('材质字段不匹配');for(const k in values)if(typeof v[k]!=='number'||!Number.isFinite(v[k])||v[k]<0||v[k]>1)throw Error('材质参数范围错误');Object.assign(values,v);for(const r of records){for(const k in values)r.uniforms[({roughness:'uRough',wear:'uWear',oil:'uOil',detail:'uDetail'})[k]].value=values[k];r.m.clearcoat=['grip','olive'].includes(r.key)?values.oil*.10:values.oil*.42;}}
