...
const NativeGeometry = (() => {
  function create(T,P,material){
    ...
    const end=b.termination;lathe(barrel,[[0,end.x0],[end.radius,end.x0],[end.radius,end.x1],[end.openingRadius,end.x1],[end.openingRadius,end.recessX],[0,end.recessX]],'x',[0,0,0],'forward-termination-visual-recess',64);
    ...
    for(let i=0;i<2;i++){
      ...
      for(const [a,b] of [rr.upperCapY,rr.lowerCapY])lathe(rear,[[0,a],[rr.capRadius*.94,a],[rr.capRadius,a+.0007],[rr.capRadius,b-.0007],[rr.capRadius*.94,b],[0,b]],'y',[x,0,z],'rear-grip-cap-'+i,48);
    }
    const fr=rr.frame;
    ...
    lathe(rear,[[0,cy.x0],[R*.85,cy.x0],[R*.98,cy.x0+d*.06],[R*.98,cy.x0+d*.14],[R*.90,cy.x0+d*.14],[R*.90,cy.x0+d*.17],[R,cy.x0+d*.17],[R,cy.x1],[0,cy.x1]],'x',[0,...cy.centerYZ],'rear-external-cylinder',64).userData.sourceNode=21;
    const uc=rr.upperControl;
    const control=rounded(uc.x0,-uc.z1,uc.x1,-uc.z0,.012);hole(control,rounded(uc.x0+uc.thickness,-uc.z1+uc.thickness,uc.x1-uc.thickness,-uc.z0-uc.thickness,.008));
    extrude(rear,control,uc.y1-uc.thickness,uc.y1,'rear-upper-control','y').userData.sourceNode=27;
    box(rear,{lo:[uc.x0,uc.y0,-.008],hi:[uc.x0+uc.thickness,uc.y1,.008]},'rear-control-visible-tab');
