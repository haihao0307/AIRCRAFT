/* W10: authored exterior appearance only; no historical material identification.
...
const SurfaceProgram=(()=>{
  const context=Object.freeze({assetId:'ODNA:MECH:ANM2:PILOT-0001',frame:'digital-reference/R02',snapshot:'W10-demo/1',worldTransform:null,historicalTime:null});
  ...
  function create(T,native){
    ...
    native.root.traverse(m=>{if(!m.isMesh)return;
      ...
      if(!domains.some(d=>d.domainId===domainId))domains.push({domainId,partId:'part/'+m.userData.semantic,regionId:'region/'+m.name,materialInstanceId:domainId+'/candidate-layer-1',interpretation:'authored-exterior-region; physical partition unverified'});
    ...
    material.customProgramCacheKey=()=> 'W10-stable-surface-1';
    ...
    function query(address,ctx=context){
      ...
      return {status:'authored-field; surface-membership-not-validated',state,roughness:presets[state].roughness+.035*grain,colorMultiplier:1+.055*grain,context};
    }
