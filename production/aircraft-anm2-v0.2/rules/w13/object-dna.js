...
const ObjectDNA=(()=>{
  ...
  const labels={receiver:'机匣外观组件',cover:'顶盖外观区域',positivePlate:'侧板外观区域',collar:'前环外观组件',barrel:'枪管与护套外观组',rear:'后端外观组件',sideHandle:'侧面操作件外观',supplyBox:'弹箱外观',supplyBelt:'弹带展示组',displayMount:'展示枪座候选',sight:'机械瞄具候选'};
  const offsets={receiver:[0,0,0],cover:[0,.35,0],positivePlate:[0,.02,.36],collar:[.25,-.2,0],barrel:[.58,.15,0],rear:[-.36,0,0],sideHandle:[0,.15,.46],supplyBox:[.25,-.15,.3],supplyBelt:[0,.22,.3],displayMount:[0,-.25,0],sight:[0,.16,0]};
  function create(T,native){
    const entries=[],frames=new Map(),byId=new Map();let amount=0,lidAmount=0,separation=0,focusedRound=1;
    const context=Object.freeze({assetId,snapshot:'W13/source-layout/1',frame:'R02-digital-reference',units:'receiver-datum separation; dimensionless',historicalTime:{status:'unknown'},worldPose:{status:'unknown'}});
    const id=key=>key.startsWith(prefix)?key:prefix+key.replace(/^W1[123]\//,'');
    function wrap(host,objects,key,label,type,partOf,poseParent,offset=[0,0,0],kind='entity-candidate',anchorOverride=null){
      ...
      const record={id:id(key),key,label,kind,typeOf:type,knowledgeVersion:'W13',partOf:id(partOf),poseParent:id(poseParent),frame:{translation:anchor.toArray(),rotationQuaternion:[0,0,0,1],units:context.units,anchorMeaning:anchorOverride?'authored display hinge; physical interface unverified':'local exterior bounds anchor; not a certified physical interface'},sourceNodes:[...new Set(meshes.flatMap(m=>m.userData.sourceNodeCandidates||[m.userData.sourceNode]).filter(n=>n!==null))],geometry:{status:'exterior-candidate',ruleFiles:key.startsWith('round/')||key.startsWith('link/')?['cartridge-program.js','supply-program.js']:key.startsWith('supply')?['supply-program.js']:['displayMount','sight'].includes(key)?['accessory-program.js']:['geometry-program.js'],meshNames:meshes.map(m=>m.name)},material:{status:'multi-reference authored preview; historical finish unknown'},knowledge:{status:'observed exterior motifs with authored interpretation'},implementation:{status:'partial'},verification:{status:'W13 coordinate and runtime checks pending; geometry, historical and user acceptance pending'},lifecycle:{historicalIdentity:'unknown',eventHistory:'unknown'},displayOffset:offset,context};
    ...
    for(const [name,host]of Object.entries(native.groups)){
      const top=wrap(host,[...host.children].filter(m=>m.isMesh),name,labels[name],name.startsWith('supply')?'supply-exterior-display':'gun-exterior-display',name.startsWith('supply')||name==='displayMount'?'exhibit':'gun','assetFrame',offsets[name],['cover','positivePlate'].includes(name)?'semantic-region-candidate':'entity-aggregate-candidate');
