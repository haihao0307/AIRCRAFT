...
regions={'receiver':([23,25,15,9],['receiver','cover','positivePlate','collar','barrel'],.24),'body':([23,25],['receiver','cover','positivePlate'],None),'positivePlate':([25],['positivePlate'],None),'collar':([15,9],['collar','barrel'],.30)}
report={'schema':'object-dna.w09-measurements/1','units':'receiver datum separation = 1; uncalibrated display coordinates','surfaceMethod':'bidirectional area-weighted surface sampling, 4000 points each, exact nearest triangle; includes hidden overlapping surfaces; diagnostic, not exterior-only acceptance','silhouetteMethod':'identical orthographic camera and pixel viewport; threshold 127; 4-neighbour boundary; distances in pixels','stats':data['stats'],'silhouettes':[],'surfaces':{},'sections':{},'datumResiduals':[]}
for m in data['masks']:
 ...
 Image.fromarray(overlay).save(root/'validation/w09'/('difference-'+m['region']+'-'+m['view']+'.png'))
 def desc(x):
...
for name,(nodes,groups,clip) in regions.items():
 ...
 stations=[-.85,-.65,-.25,-.08] if name in ('body','receiver') else ([-.9,-.7,-.55] if name=='positivePlate' else [.02,.07,.18])
 sections=[]
 for x in stations:
  row={'planeX':x}
...
(root/'validation/W09_MEASUREMENTS.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
