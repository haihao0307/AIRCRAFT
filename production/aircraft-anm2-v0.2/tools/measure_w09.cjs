...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 const page=await browser.newPage({viewport:{width:1600,height:1100}});await page.goto(pathToFileURL(path.join(root,'releases/w09/AIRCRAFT_ANM2_OBJECT_DNA_W09.html')).href);await page.waitForFunction(()=>window.reviewReady);
 ...
 for(const region of ['receiver','body','positivePlate','collar'])for(const view of ['side','reverse','top','bottom','front','back']){
 ...
 await new Promise((resolve,reject)=>{const p=spawn('G:/AIRCRAFT/object-dna-runtime/Scripts/python.exe',[path.join(__dirname,'measure_w09.py')],{cwd:root,stdio:['pipe','inherit','inherit']});p.on('exit',c=>c?reject(Error('measure '+c)):resolve());p.stdin.end(JSON.stringify(payload));});
