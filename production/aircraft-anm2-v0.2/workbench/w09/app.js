(() => {
  ...
  let reference=null,activeRegion='receiver',activeView='oblique',mode='neutral',showDatums=false,ready=false;
  ...
  function resize(){const w=hosts[0].clientWidth,h=hosts[0].clientHeight;for(let i=0;i<2;i++)renderers[i].setSize(hosts[i].clientWidth,hosts[i].clientHeight,false);const aspect=w/h;camera.left=-frameSize*aspect/2;camera.right=frameSize*aspect/2;camera.top=frameSize/2;camera.bottom=-frameSize/2;camera.updateProjectionMatrix();render();}
  function fit(view){
    ...
    const size=box.getSize(new T.Vector3()),aspect=hosts[0].clientWidth/hosts[0].clientHeight;
  ...
  function setRegion(id){
    ...
    material.clippingPlanes=clips;maskMaterial.clippingPlanes=clips;material.needsUpdate=true;maskMaterial.needsUpdate=true;
    $('native-caption').textContent=q.title;fit(activeView);
  }
  function setMode(name){mode=name;const mat=mode==='silhouette'?maskMaterial:material;
    native.root.traverse(m=>{if(m.isMesh)m.material=mat;});if(reference)reference.meshes.forEach(m=>m.material=mat);
    $('neutral').setAttribute('aria-pressed',String(mode==='neutral'));$('silhouette').setAttribute('aria-pressed',String(mode==='silhouette'));render();
  }
  function report(){
    ...
    $('report-content').innerHTML='<p>本版审阅范围是机匣、顶盖/侧板层次与枪管根部过渡。整枪上下文不等于整枪完成。所有比例来自数字参考外观，没有制造尺寸标定。</p>'+
      '<table><tr><th>核验项</th><th>本轮结果</th></tr>'+e.map(row=>'<tr><td>'+row.label+'</td><td>'+row.value+'</td></tr>').join('')+'</table>'+
      '<p>待完成：B24 具体型号、枪位、日期、瞄具、箱体及外接关系；历史材质与状态动作。用户视觉验收、生产就绪均保持 false。</p>'+
  ...
  async function load(file){
    ...
    try{const parsed=await ReferenceInput.read(file);const next=ReferenceTwin.create(T,parsed,mode==='neutral'?material:maskMaterial,DATUM_RECORD);
      ...
      $('empty').classList.add('hidden');$('ref-state').textContent='IDENTITY VERIFIED';$('ref-caption').textContent='原件已核验 · 28 节点 / 13 网格';$('load-reference').textContent='重新选择原件';setRegion(activeRegion);window.referenceReady=true;
  ...
  controls.addEventListener('change',render);new ResizeObserver(()=>{if(ready)fit(activeView);}).observe(hosts[0]);
  ...
  window.review={setRegion,setView:fit,setMode,render,stats,
    referenceAudit:()=>reference?.audit(),
    current:()=>({region:activeRegion,view:activeView,mode,referenceLoaded:!!reference,frame:{position:camera.position.toArray(),target:controls.target.toArray(),left:camera.left,right:camera.right,top:camera.top,bottom:camera.bottom,zoom:camera.zoom}}),
  ...
  report();ready=true;setRegion('receiver');window.reviewReady=true;
})();
