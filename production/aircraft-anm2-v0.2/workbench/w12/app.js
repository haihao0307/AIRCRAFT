(() => {
  ...
  const pmrem=new T.PMREMGenerator(renderers[1]),environment=pmrem.fromScene(studio,.06,.1,30);surface.materials.forEach(m=>{m.envMap=environment.texture;m.envMapIntensity=.85;});pmrem.dispose();studio.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});
  ...
  const regions={
    assembly:{nodes:[6,7,9,11,15,17,19,21,23,25,27],groups:Object.keys(native.groups),bounds:[[-1.18,-1.30,-.18],[1.74,.22,3.0]],title:'枪体与供弹外观 · 原件布局候选'},
    ...
    full:{nodes:[9,15,17,19,21,23,25,27],groups:Object.keys(native.groups).filter(n=>!n.startsWith('supply')),bounds:[[-1.16,-.18,-.17],[1.73,.18,.30]],title:'整枪外观上下文 · 尚未整体验收'},
  ...
  const views={oblique:[1,.65,1.7],rearOblique:[-1,.65,1.7],side:[0,0,1],reverse:[0,0,-1],top:[0,1,0],bottom:[0,-1,0],front:[1,0,0],back:[-1,0,0]};
  let frameSize=1;
  ...
  function fit(view){
    activeView=view;const spatial=['oblique','rearOblique'].includes(view);camera=spatial?perspective:orthographic;controls.object=camera;controls.enableRotate=spatial;
    ...
    if(spatial){
      ...
      if(!sourceOnly&&!['round','box','muzzle'].includes(activeRegion))collect(native.root);if(reference&&(sourceOnly||comparing))collect(reference.root);
  ...
  function setRegion(id){
    if(sourceOnly)setSourceOnly(false);activeRegion=id;$('region').value=id;const q=regions[id];
    for(const [name,g] of Object.entries(native.groups))g.visible=q.groups.includes(name);
    native.groups.supplyBelt.traverse(m=>{if(m.isMesh)m.visible=id!=='round'||m.userData.instanceIndex===1;});
    ...
    surface.materials.forEach(m=>{m.clippingPlanes=clips;m.needsUpdate=true;});material.clippingPlanes=clips;maskMaterial.clippingPlanes=clips;material.needsUpdate=true;maskMaterial.needsUpdate=true;
  ...
  function setMode(name){mode=name;if(name==='surface')setCompare(false);const mat=mode==='silhouette'?maskMaterial:mode==='surface'?surface.material:material;
    ...
    native.root.traverse(m=>{if(m.isMesh)m.material=mode==='surface'?surface.matForMesh(m):mat;});if(reference)reference.meshes.forEach(m=>m.material=mode==='silhouette'?maskMaterial:material);
  ...
  function setCompare(enabled){if(sourceOnly)setSourceOnly(false);if(enabled){setExplode(0);setSeparation(0);setLid(0);}comparing=enabled;stage.classList.toggle('solo',!enabled);$('compare').setAttribute('aria-pressed',String(enabled));if(enabled&&mode==='surface')setMode('neutral');fit(activeView);}
  ...
  function setSeparation(value){if(value>0)soloDisplay();dna.setSeparation(value,1);$('separation').value=value;$('separation-value').textContent=value?'分体 '+Math.round(value*100)+'%':'合拢';render();}
  function setLid(value){if(value>0)soloDisplay();dna.setLid(value);$('lid').value=value;$('lid-value').textContent=value?'开启 '+Math.round(value*100)+'%':'闭合';render();}
  $('separation').oninput=e=>setSeparation(Number(e.target.value));$('lid').oninput=e=>setLid(Number(e.target.value));
  document.querySelectorAll('[data-region]').forEach(b=>b.onclick=()=>{dna.setExplode(0);$('explode').value=0;$('explode-value').textContent='合拢';setRegion(b.dataset.region);setMode('surface');});
  ...
  $('export-dna').onclick=()=>{const blob=new Blob([JSON.stringify(dna.packet(),null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='AIRCRAFT_ANM2_W12_ObjectDNA.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
  ...
  function report(){
    ...
    $('report-content').innerHTML='<p>W12 将弹药分为独立弹壳、弹头与链节，细分枪管和护套，并加入箱盖展示开合。每个对象具有稳定身份、组成与坐标关系。所有尺寸为数字参考比例。</p>'+ 
      '<table><tr><th>核验项</th><th>本轮结果</th></tr>'+e.map(row=>'<tr><td>'+row.label+'</td><td>'+row.value+'</td></tr>').join('')+'</table>'+ 
      '<p>分体和开合均为新编写的展示控制；已检查的来源没有动画。没有实现击发、内部机构或真实拆装路径。弹箱内部装载未知，当前显示空腔。</p>'+ 
      ...
      '<p>B24 枪位、安装、日期、瞄具和历史材质仍待证据。几何与用户验收待定，生产就绪保持 false。</p>';
  }
  async function load(file){
    $('loading').classList.add('visible');$('load-reference').disabled=true;$('notice').classList.remove('visible');
    try{let next;if(file.size===777908)next=await Cal50Reference.create(T,file,material);else{const parsed=await ReferenceInput.read(file);next=ReferenceTwin.create(T,parsed,material,DATUM_RECORD);next.kind='original';}
      ...
      $('empty').classList.add('hidden');$('ref-state').textContent='IDENTITY VERIFIED';$('ref-caption').textContent=reference.kind==='cal50'?'单弹参考已核验 · 5 节点 / 3 网格':'原件已核验 · 28 节点 / 13 网格';$('source-all').textContent=reference.kind==='cal50'?'完整参考 · 3 网格':'完整原件 · 13 网格';$('load-reference').textContent='重新选择原件';const wasSourceOnly=sourceOnly;setRegion(reference.kind==='cal50'?'round':activeRegion==='round'?'assembly':activeRegion);window.referenceReady=true;if(wasSourceOnly)setSourceOnly(true);else setCompare(true);
  ...
  window.review={muzzleAudit:()=>{native.root.updateMatrixWorld(true);const mesh=native.root.getObjectByName('barrel-hollow-tube'),ray=new T.Raycaster(new T.Vector3(1.8,0,0),new T.Vector3(-1,0,0));ray.far=1.8;return {centerHits:ray.intersectObject(mesh).length};},setLid,setSeparation,setExplode,selectObject,setSourceOnly,dnaPacket:dna.packet,dnaPosition:dna.position,
    ...
    current:()=>({region:activeRegion,view:activeView,mode,comparing,sourceOnly,projection:camera.type,expansion:dna.amount(),lid:dna.lid(),separation:dna.separation(),referenceKind:reference?.kind,selectedId,surfaceState:surface.state(),referenceLoaded:!!reference,frame:{position:camera.position.toArray(),target:controls.target.toArray(),left:camera.left,right:camera.right,top:camera.top,bottom:camera.bottom,zoom:camera.zoom}}),
  ...
  report();ready=true;setRegion('assembly');setMode('surface');window.reviewReady=true;
})();
