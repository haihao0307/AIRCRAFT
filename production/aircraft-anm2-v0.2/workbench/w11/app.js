(() => {
  ...
  const brass=surface.material.clone();brass.color.setHex(0xa58a50);brass.metalness=.75;brass.roughness=.43;brass.onBeforeCompile=surface.material.onBeforeCompile;brass.customProgramCacheKey=surface.material.customProgramCacheKey;
  ...
  function updateLabels(){
    ...
    for(const e of dna.entries){if(e.id!==selectedId&&e.poseParent!=='W11/assetFrame')continue;
      if(hosts[1].clientWidth<500&&e.id!==selectedId&&!['W11/receiver','W11/barrel','W11/rear','W11/supplyBox','W11/supplyBelt'].includes(e.id))continue;
  ...
  function fit(view){
    ...
    if(spatial){
      ...
      if(!sourceOnly)collect(native.root);if(reference&&(sourceOnly||comparing))collect(reference.root);
      ...
      if(points.length)camera.position.copy(center).addScaledVector(direction,distance);
    }
  ...
  function setRegion(id){
    ...
    brass.clippingPlanes=clips;brass.needsUpdate=true;surface.material.clippingPlanes=clips;surface.material.needsUpdate=true;material.clippingPlanes=clips;maskMaterial.clippingPlanes=clips;material.needsUpdate=true;maskMaterial.needsUpdate=true;
  ...
  function setMode(name){mode=name;if(name==='surface')setCompare(false);const mat=mode==='silhouette'?maskMaterial:mode==='surface'?surface.material:material;
    brass.envMap=environment.texture;brass.envMapIntensity=surface.material.envMapIntensity;
    native.root.traverse(m=>{if(m.isMesh)m.material=mode==='surface'&&m.userData.appearance==='brass'?brass:mat;});if(reference)reference.meshes.forEach(m=>m.material=mode==='silhouette'?maskMaterial:material);
  ...
  function setCompare(enabled){if(sourceOnly)setSourceOnly(false);if(enabled)setExplode(0);comparing=enabled;stage.classList.toggle('solo',!enabled);$('compare').setAttribute('aria-pressed',String(enabled));if(enabled&&mode==='surface')setMode('neutral');fit(activeView);}
  function setSourceOnly(enabled){
    ...
    if(reference)reference.meshes.forEach(m=>m.visible=enabled||regions[activeRegion].nodes.includes(m.userData.sourceNode));
    fit(activeView);
  }
  ...
  function selectObject(id){
    selectedId=id;const e=dna.get(id);selectionBox.visible=!!e;$('object-select').value=id;
  ...
  $('export-dna').onclick=()=>{const blob=new Blob([JSON.stringify(dna.packet(),null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='AIRCRAFT_ANM2_W11_ObjectDNA.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
  ...
  function report(){
    ...
    $('report-content').innerHTML='<p>W11 默认正常透视，固定方向使用正交测量。修整后端翼状件与端部外观，加入原件弹箱和 58 个弹药展示实例；提供对象身份、相对坐标与展示展开。供弹形状为候选外观，未取得历史或用户验收。所有比例来自数字参考外观，没有制造尺寸标定。</p>'+
      '<table><tr><th>核验项</th><th>本轮结果</th></tr>'+e.map(row=>'<tr><td>'+row.label+'</td><td>'+row.value+'</td></tr>').join('')+'</table>'+
      '<p>待完成：B24 具体型号、枪位、日期、瞄具、安装与外接关系；历史材质与状态动作。当前两种材质只是候选外观，不表示真实涂层替换事件；未接入世界内核，未验证重新网格化映射。用户视觉验收、生产就绪均保持 false。</p>'+
      '<p>对象关系：总装包含枪体、弹箱与弹带展示组。枪体有 7 个外观组；后端下设握把 A / B、翼状件与圆筒。58 个弹药实例各有身份和相对坐标，carriedBy 指向弹带，组成归属仍为总装。组成关系、类型继承和坐标父级分别记录，网格数量不等于实物部件数量。</p>'+
      '<table><tr><th>未完成 / 待确认项</th><th>状态</th></tr><tr><td>枪管与护套细分、内部机构</td><td>未实现</td></tr><tr><td>B24 枪位、安装、瞄具与外接关系</td><td>适用性待确认</td></tr><tr><td>世界位置、历史日期、处置路径</td><td>未知</td></tr><tr><td>历史材质、实体沿革</td><td>未知</td></tr><tr><td>跨系统 Object DNA 交换</td><td>本项目草案，非通用标准</td></tr></table>'+
      '<p>原件共 13 网格，总装对照纳入 11 个；另外 2 个为独立摆放样件，可由“完整原件”查看。原件只在本地内存读取。参考作者：Misja van Laatum；原件许可元数据：CC BY 4.0。当前规则生成体属于新的外观解释。</p>';
  }
  async function load(file){
    ...
    try{const parsed=await ReferenceInput.read(file);const next=ReferenceTwin.create(T,parsed,mode==='silhouette'?maskMaterial:material,DATUM_RECORD);
      ...
      $('empty').classList.add('hidden');$('ref-state').textContent='IDENTITY VERIFIED';$('ref-caption').textContent='原件已核验 · 28 节点 / 13 网格';$('load-reference').textContent='重新选择原件';const wasSourceOnly=sourceOnly;setRegion(activeRegion);window.referenceReady=true;if(wasSourceOnly)setSourceOnly(true);else setCompare(true);
  ...
  window.review={setExplode,selectObject,setSourceOnly,dnaPacket:dna.packet,dnaPosition:dna.position,
    ...
    current:()=>({region:activeRegion,view:activeView,mode,comparing,sourceOnly,projection:camera.type,expansion:dna.amount(),selectedId,surfaceState:surface.state(),referenceLoaded:!!reference,frame:{position:camera.position.toArray(),target:controls.target.toArray(),left:camera.left,right:camera.right,top:camera.top,bottom:camera.bottom,zoom:camera.zoom}}),
