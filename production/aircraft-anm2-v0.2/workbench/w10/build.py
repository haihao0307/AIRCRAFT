...
files=[vendor/'three-controls.runtime.js',vendor/'reference-input.js',ROOT/'rules/w10/geometry-program.js',ROOT/'rules/w10/surface-program.js',ROOT/'rules/reference-twin.js',HERE/'app.js']
params=json.loads(read(ROOT/'rules/w10/EXTERIOR_RECIPE_R01.json'))
datums=json.loads(read(ROOT/'DATUM_REGISTRATION_R02.json'))
reportPath=ROOT/'validation/W10_PREFLIGHT.json'
...
out=ROOT/'releases/w10/AIRCRAFT_ANM2_OBJECT_DNA_W10.html';out.parent.mkdir(parents=True,exist_ok=True);out.write_text(html,encoding='utf-8')
receipt=dict(schema='object-dna.review-build/0.2',path=out.relative_to(ROOT).as_posix(),bytes=out.stat().st_size,sha256=hashlib.sha256(out.read_bytes()).hexdigest(),
 referenceEmbedded=False,previousGeometryInherited=True,geometryBaseline="W09",visualAcceptance=False,productionReady=False,
 sources={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in files+[HERE/'template.html',ROOT/'rules/w10/EXTERIOR_RECIPE_R01.json',ROOT/'DATUM_REGISTRATION_R02.json']})
