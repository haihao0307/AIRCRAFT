...
const outputDir = path.resolve(root, process.env.WM_V016_EVIDENCE_DIR ?? 'reports/weapons-mother/v016-browser');
const baseUrl = process.env.WM_V016_BASE_URL ?? 'http://127.0.0.1:8765/preview/weapons-mother/b24-m2-aircraft-v016/index.html';
const stressShots = Number(process.env.WM_V016_STRESS_SHOTS ?? 2000);

await fs.mkdir(outputDir, { recursive: true });
const browser = await chromium.launch({
  headless: true,
  executablePath: process.env.WM_V015_CHROME ?? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
});
...
try {
  ...
  check('muzzle lateral datum is calibrated', alignment?.lateralAlignmentPass === true && alignment?.perpendicularMuzzleOffsetMeters <= 0.012, { offset: alignment?.perpendicularMuzzleOffsetMeters, pass: alignment?.lateralAlignmentPass });
  check('rear lateral datum is calibrated', alignment?.perpendicularRearOffsetMeters <= 0.012, alignment?.perpendicularRearOffsetMeters);
  ...
  check('feed terminal cartridge axis reaches gun axis', alignment?.feedTerminalCartridgeDot >= 0.999, alignment?.feedTerminalCartridgeDot);
  check('source-axis round samples stay across the belt path', alignment?.roundAxisPass === true, alignment?.roundAxisSamples);
  check('complete rear sight source neighborhood is present', alignment?.sightComponentCount >= 8, alignment?.sightComponentCount);
  check('belt uses complete source Object_11/Object_12 arrays without procedural rails', alignment?.sourceBeltPass === true && alignment?.sourceOriginalBelt?.sourceNodeIndices?.join(',') === '11,12', alignment?.sourceOriginalBelt);
  check('feed socket is inset from source terminal', Math.abs((alignment?.feedSocketInsetMeters ?? 0) - 0.035) < 1e-8, alignment?.feedSocketInsetMeters);
  check('display lift is removed from physical alignment', Math.abs(alignment?.displayLiftMeters ?? Infinity) < 1e-8, alignment?.displayLiftMeters);
  check('display elevation is a controlled 2 degree presentation pitch', Math.abs((alignment?.presentationElevationDegrees ?? Infinity) - 2) < 1e-8 && alignment?.presentationPass === true, alignment?.presentationElevationDegrees);
  check('source container is horizontal with its local Z as feed heading', alignment?.boxHorizontalPass === true, { boxHorizontalPass: alignment?.boxHorizontalPass, orientation: alignment?.alignments?.['waist-starboard']?.boxOrientation });
  check('box outlet is the route origin', alignment?.boxOutletErrorMeters < 0.02, alignment?.boxOutletErrorMeters);
  check('box outlet lies on the container opening plane', alignment?.boxOutletInContainerPass === true, { boxExitLocal: alignment?.alignments?.['waist-starboard']?.boxExitLocal, pass: alignment?.boxOutletInContainerPass });
  ...
  const report = {
    schema: 'haihao.aircraft/weapons-mother-v016-browser-qa@1.0.0',
  ...
  console.log(`Weapons Mother V016 browser QA: ${report.status} (${checks.filter((item) => item.pass).length}/${checks.length})`);
