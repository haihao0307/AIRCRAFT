...
const outputDir = path.resolve(root, process.env.WM_V015_EVIDENCE_DIR ?? 'reports/weapons-mother/v015-browser');
const baseUrl = process.env.WM_V015_BASE_URL ?? 'http://127.0.0.1:8765/preview/weapons-mother/b24-m2-aircraft-v015/index.html';
const stressShots = Number(process.env.WM_V015_STRESS_SHOTS ?? 2000);
...
try {
  ...
  const report = {
    schema: 'haihao.aircraft/weapons-mother-v015-browser-qa@1.0.0',
  ...
  console.log(`Weapons Mother V015 browser QA: ${report.status} (${checks.filter((item) => item.pass).length}/${checks.length})`);
