...
const outputDir = path.resolve(root, process.env.WM_V014_EVIDENCE_DIR ?? 'reports/weapons-mother/v014-browser');
const baseUrl = process.env.WM_V014_BASE_URL ?? 'http://127.0.0.1:8765/preview/weapons-mother/b24-m2-aircraft-v014/index.html';
const stressShots = Number(process.env.WM_V014_STRESS_SHOTS ?? 2000);

await fs.mkdir(outputDir, { recursive: true });
const browser = await chromium.launch({
  headless: true,
  executablePath: process.env.WM_V014_CHROME ?? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
});
...
try {
  ...
  check('display elevation is removed from physical alignment', Math.abs(alignment?.presentationElevationDegrees ?? Infinity) < 1e-8, alignment?.presentationElevationDegrees);
  ...
  const report = {
    schema: 'haihao.aircraft/weapons-mother-v014-browser-qa@1.0.0',
  ...
  console.log(`Weapons Mother V014 browser QA: ${report.status} (${checks.filter((item) => item.pass).length}/${checks.length})`);
