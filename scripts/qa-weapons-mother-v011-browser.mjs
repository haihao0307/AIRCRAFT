...
const outputDir = path.resolve(root, process.env.WM_V011_EVIDENCE_DIR ?? 'reports/weapons-mother/v011-browser');
const baseUrl = process.env.WM_V011_BASE_URL ?? 'http://127.0.0.1:8765/preview/weapons-mother/b24-m2-aircraft-v011/index.html';
const stressShots = Number(process.env.WM_V011_STRESS_SHOTS ?? 2000);

await fs.mkdir(outputDir, { recursive: true });
const browser = await chromium.launch({
  headless: true,
  executablePath: process.env.WM_V011_CHROME ?? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
});
...
try {
  ...
  check('node 799 route endpoint meets gun feed socket', alignment?.feedEndpointErrorMeters < 0.02, alignment?.feedEndpointErrorMeters);
  ...
  const report = {
    schema: 'haihao.aircraft/weapons-mother-v011-browser-qa@1.0.0',
  ...
  console.log(`Weapons Mother V011 browser QA: ${report.status} (${checks.filter((item) => item.pass).length}/${checks.length})`);
