...
const outputDir = path.resolve(root, process.env.WM_V010_EVIDENCE_DIR ?? 'reports/weapons-mother/v010-browser');
const baseUrl = process.env.WM_V010_BASE_URL ?? 'http://127.0.0.1:8765/preview/weapons-mother/b24-m2-aircraft-v004/index.html';
const stressShots = Number(process.env.WM_V010_STRESS_SHOTS ?? 2000);

await fs.mkdir(outputDir, { recursive: true });
const browser = await chromium.launch({
  headless: true,
  executablePath: process.env.WM_V010_CHROME ?? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
});
...
try {
  ...
  check('muzzle and rear landmarks are coincident', alignment?.maxPositionError < 1e-5, alignment?.maxPositionError);
  check('node 802 local +Z controls roll', alignment?.minimumRollDot > 0.999999, alignment?.minimumRollDot);
  check('node 811 datum meets real floor', alignment?.braceFloorErrorMeters < 1e-5, alignment?.braceFloorErrorMeters);
  check('node 799 route endpoint meets gun feed socket', alignment?.feedEndpointErrorMeters < 1e-5, alignment?.feedEndpointErrorMeters);
  ...
  check('settled collision queue is bounded', stress.recentSettledColliders <= 72, stress.recentSettledColliders);
  ...
  const report = {
    schema: 'haihao.aircraft/weapons-mother-v010-browser-qa@1.0.0',
  ...
  console.log(`Weapons Mother V010 browser QA: ${report.status} (${checks.filter((item) => item.pass).length}/${checks.length})`);
