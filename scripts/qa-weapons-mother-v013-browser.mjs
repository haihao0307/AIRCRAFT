...
const outputDir = path.resolve(root, process.env.WM_V013_EVIDENCE_DIR ?? 'reports/weapons-mother/v013-browser');
const baseUrl = process.env.WM_V013_BASE_URL ?? 'http://127.0.0.1:8765/preview/weapons-mother/b24-m2-aircraft-v013/index.html';
const stressShots = Number(process.env.WM_V013_STRESS_SHOTS ?? 2000);

await fs.mkdir(outputDir, { recursive: true });
const browser = await chromium.launch({
  headless: true,
  executablePath: process.env.WM_V013_CHROME ?? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
});
...
try {
  ...
  check('display lift is the documented 50 mm', Math.abs((alignment?.displayLiftMeters ?? 0) - 0.05) < 1e-6, alignment?.displayLiftMeters);
  check('display elevation is the documented 4 degrees', Math.abs((alignment?.presentationElevationDegrees ?? 0) - 4) < 1e-6, alignment?.presentationElevationDegrees);
  ...
  const report = {
    schema: 'haihao.aircraft/weapons-mother-v013-browser-qa@1.0.0',
  ...
  console.log(`Weapons Mother V013 browser QA: ${report.status} (${checks.filter((item) => item.pass).length}/${checks.length})`);
