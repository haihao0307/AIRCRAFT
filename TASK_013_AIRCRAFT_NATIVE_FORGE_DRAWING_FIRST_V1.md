...
5. 锁定参考模型保持不变
6. Image2ThreeJS 和同类外部重建工具默认关闭
7. 禁止调用外部工具推断飞机结构、尺寸、机构和表面
...
1. img2threejs 不出现在 package 依赖中
2. img2threejs 不出现在运行时 import 中
3. img2threejs 不出现在 Actions 执行命令中
4. 自动调用状态为关闭
5. 所有分析和批准权限为 false
6. 只有用户明确指令能够临时改变单项状态
