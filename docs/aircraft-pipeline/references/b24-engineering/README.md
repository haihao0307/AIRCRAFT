...
README 当前标注版本为 `1.5.1`，许可证为 Apache 2.0。它通过程序化几何、材质和运行时层级，把参考图重建为代码生成的 Three.js 模型。可选多视图流程会相交至少两张正投影轮廓，并把不可见区域保留为低置信度。

当前状态：E 级工具候选。适合生成网页代理模型、检查轮廓、拆分层级、铆钉与细节清单。所有尺寸、曲面、材料和变型适用性继续由原厂工程来源控制。
...
论文与仓库描述了一条从栅格正投影工程图生成可编辑 CadQuery 代码，并进一步转换成 3D CAD 的研究流程。它还包含投影比较与代码自修正。

当前状态：E 级自动化研究工具。只有在 B-24 图纸完成尺寸标定、视图对齐和变型分组后，才进入隔离测试。生成结果属于重建假设，需要与原厂尺寸、剖面和图号逐项比较。
