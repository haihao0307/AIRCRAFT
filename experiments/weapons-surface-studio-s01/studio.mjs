...
function setView(view,animate=true){currentView=view;controls.autoRotate=false;$('rotate').setAttribute('aria-pressed','false');document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('selected',b.dataset.view===view));
 ...
 if(aspect<.72&&view==='hero'){direction.set(-1.45,-2.8,1.45);distance*=1.05;target.z+=.03;}
 ...
 if(animate&&!matchMedia('(prefers-reduced-motion: reduce)').matches){poseAnimation={start:performance.now(),from:camera.position.clone(),to:dest,tf:controls.target.clone(),tt:target};}else{poseAnimation=null;camera.position.copy(dest);controls.target.copy(target);controls.update();}
...
async function start(){if(!window.DecompressionStream)throw new Error('需要支持 DecompressionStream 的现代浏览器');const raw=bytes64($('packed-asset').textContent.trim());const stream=new Blob([raw]).stream().pipeThrough(new DecompressionStream('gzip'));asset=JSON.parse(await new Response(stream).text());
 ...
 document.querySelector('.stage-heading p').textContent='V016 历史样本 · 保留源几何的材质实验。';$('source-receipt').textContent=JSON.stringify(asset.receipt,null,2);$('integrity-summary').textContent=`${asset.receipt.accessorCount} 项校验通过`;$('triangle-count').textContent=(asset.receipt.triangles/1000).toFixed(1)+'k';$('status').textContent='源几何保留 · 表面候选就绪';$('loading').classList.add('hidden');ready=true;document.body.dataset.ready='true';
