# 最新接续：R16 主翼拼缝与局部分色复查（2026-09-09）

当前分支 `feature/b24-wing-seams-r16`。[R16固定公网预览与验收](docs/wing-seams-r16/PUBLIC_PREVIEW.md)已完成，固定代码 `47ba8a21676b49e6a1d4d6d4285c28ba1f2f538a`，入口 `b24-wing-seams-r16.html`。主翼上下表面分别按Airfix第8/17/18页长板关系和明确起止重排；平尾根部上面灰斑、左右主轮上方发动机罩色块修正；四处排气出口积碳随服役增加，维护可减轻。重点按钮可直达这几处。

[知识正文](docs/wing-seams-r16/KNOWLEDGE.md)记录端点、来源和诊断证据。线位配准/钉距/磨耗时间是外观示意，其他机身缝位未改称制造线。原始数组、348网格/1784节点及军徽锚点保持，禁止复用已作废105段。展示默认正常透视，测量使用标注正交；前后、缩放和公网资源已验收。后续遵守固定版本公网HTTPS与同尺度相机规则；旧版不覆盖。以下是历史接续。
