...
import {isDetailSurface} from './b24-r8-detail-materials.js';
...
export function createSkinSystem(aircraft,renderer){
  ...
  const shaderCode=`
    ...
    if(engineRadius>.48 && engineRadius<.94 && p.z>-.2 && p.z<3.12 && abs(p.x)>2.4){
      vec3 radial=vec3(p.x-sign(p.x)*engineX,p.y-engineY,0.0);
      if(dot(cameraPosition-p,radial)<0.0)surfacePaint=skinLower*.55;
    }
  ...
  for(const mesh of paintMeshes){
    ...
    mat.customProgramCacheKey=()=> 'b24-r8-structural-paint-v1';mat.needsUpdate=true;
  }
  const placements=[];
  ...
  return {paintMeshes,placements,meshNode,bodySectionAt,bodySections,wingTriangles,
    ...
    audit:{version:'R8',visualAcceptance:false,productionReady:false,referenceJPEGIdentityVerified:false,
