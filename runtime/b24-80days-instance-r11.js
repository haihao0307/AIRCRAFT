...
const FRAME_REGION=Object.freeze({zMin:7.46,zMax:10.05,yMin:-1.52,yMax:1.23,xMin:.045,zBars:[9.70,8.56,7.58],yBars:[1.10,.57,-.04,-.77,-1.42]});
...
if(e04Visible>.5 && vSkinWorld.x>${FRAME_REGION.xMin.toFixed(3)}){
  ...
  float zFrames=max(max(1.0-smoothstep(0.028,0.072,abs(vSkinWorld.z-${FRAME_REGION.zBars[0].toFixed(2)})),1.0-smoothstep(0.028,0.072,abs(vSkinWorld.z-${FRAME_REGION.zBars[1].toFixed(2)}))),1.0-smoothstep(0.028,0.072,abs(vSkinWorld.z-${FRAME_REGION.zBars[2].toFixed(2)})));
  float yFrames=max(max(1.0-smoothstep(0.028,0.072,abs(vSkinWorld.y-${FRAME_REGION.yBars[0].toFixed(2)})),1.0-smoothstep(0.028,0.072,abs(vSkinWorld.y-${FRAME_REGION.yBars[1].toFixed(2)}))),max(max(1.0-smoothstep(0.028,0.072,abs(vSkinWorld.y+${Math.abs(FRAME_REGION.yBars[2]).toFixed(2)})),1.0-smoothstep(0.028,0.072,abs(vSkinWorld.y+${Math.abs(FRAME_REGION.yBars[3]).toFixed(2)}))),1.0-smoothstep(0.028,0.072,abs(vSkinWorld.y+${Math.abs(FRAME_REGION.yBars[4]).toFixed(2)}))));
...
`;
    ...
    mat.customProgramCacheKey=()=>`${previousKey?previousKey():''}|80days-r11-forward-mouth-red-frame-v6`;
...
try{
  ...
  window.__B24_80DAYS_R11__={schema:'haihao.aircraft/80-days-instance-runtime@11.3',mother:{id:'b24-generic-mother-01',freezeCommit:'636f26ec102680b4154a6f9dca0cf49fc951f51e',modified:false},sourceArtwork:{sha256:SOURCE_SHA,compositeSha256:R11_ART_SHA256,redrawn:false,transport:'inline-exact-png-data-url',logicalXRange:[R11_ART_MIN_X,R11_ART_MIN_X+R11_ART_WIDTH],components:['mouth-forward','robby-forward','title-enlarged-raised','dice','eye','bombs','japanese-flags']},reference:{id:'E04-left',sha256:E04.sha256},windowFrames:{red:true,glassTransparent:true,mode:'forward-port-frame-bands',region:FRAME_REGION},placement:{...placement},visualAcceptance:false,productionReady:false};
