...
import {BAY_POSES_R8 as BAY_POSES_R9} from './b24-r8-bay-poses.js';
import {applyDetailMaterials} from './b24-r9-detail-materials.js';
import {createSkinSystem,makeInsigniaTexture,makeDecal,makeReferenceSeams} from './b24-r9-skin-system.js';
...
function closeRepairedBombBay(){
  for(const part of BAY_POSES_R9.parts)applyAssetMatrix(part.sourceNode,part.closed);
  aircraft.group.updateMatrixWorld(true);
  const inverse=aircraft.group.matrixWorld.clone().invert();let maxMatrixError=0;
  for(const part of BAY_POSES_R9.parts){
  ...
  window.__B24_BAY_R9__={parts:BAY_POSES_R9.parts.length,maxMatrixError,sourcePackage:BAY_POSES.sourcePackage,sourceSHA256:BAY_POSES.sourceSHA256,sideSillFit:BAY_POSES_R9.groups,staticPoseOnly:true};
}
...
function buildInsignia(){
  ...
  $('#decalState').textContent='四处军徽位置保持本轮前确认结果；R9 未调整锚点、尺寸或方向。';
  window.__B24_GENERIC_R9__={...skinSystem.audit,results,closedDoorNodes:[764,767,773,776],split:0,colors:{upper:'#5e5839',lower:'#8b8c97'}};
  window.__B24_R9_QA__={
...
const detailViews={
  ...
  engine:{position:[5,0,6],target:[3.378,0,2.5],label:'发动机罩：前缘橄榄色环绕下唇；后方下侧灰色。发动机内部沿用 R8。'},
