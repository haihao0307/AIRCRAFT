...
import {BAY_POSES_R8 as BAY_POSES_R11} from './b24-r8-bay-poses.js';
...
let aircraft,insigniaGroup,guideGroup,split=0,paintMeshes=[],skinSystem;const upper=new THREE.Color(0x5e5839),lower=new THREE.Color(0x8b8c97);
...
function closeRepairedBombBay(){
  for(const part of BAY_POSES_R11.parts)applyAssetMatrix(part.sourceNode,part.closed);
  aircraft.group.updateMatrixWorld(true);
  const inverse=aircraft.group.matrixWorld.clone().invert();let maxMatrixError=0;
  for(const part of BAY_POSES_R11.parts){
  ...
  window.__B24_BAY_R11__={parts:BAY_POSES_R11.parts.length,maxMatrixError,sourcePackage:BAY_POSES.sourcePackage,sourceSHA256:BAY_POSES.sourceSHA256,sideSillFit:BAY_POSES_R11.groups,staticPoseOnly:true};
}
...
function buildInsignia(){
  ...
  $('#decalState').textContent='四处军徽位置保持本轮前确认结果；R11 未调整锚点、尺寸或方向。';
  window.__B24_GENERIC_R11__={...skinSystem.audit,results,closedDoorNodes:[764,767,773,776],split:0,colors:{upper:'#5e5839',lower:'#8b8c97'}};
  let wingIsolationRestore,propVisibilityRestore;
  window.__B24_R11_QA__={
