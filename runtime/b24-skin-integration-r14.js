...
import {createB24JointDemo} from './b24-joint-demo-r14.js';
import {NativeAircraft} from './native-aircraft.js';
import {BAY_POSES} from './b24-r7-bay-poses.js';
import {BAY_POSES_R8 as BAY_POSES_R14} from './b24-r8-bay-poses.js';
import {applyDetailMaterials} from './b24-r10-detail-materials.js';
import {createSkinSystem,makeInsigniaTexture,makeDecal} from './b24-r11-skin-system.js';
...
let jointDemo;let aircraft,insigniaGroup,guideGroup,split=0,paintMeshes=[],skinSystem;const upper=new THREE.Color(0x5e5839),lower=new THREE.Color(0x8b8c97);
...
function closeRepairedBombBay(){
  for(const part of BAY_POSES_R14.parts)applyAssetMatrix(part.sourceNode,part.closed);
  aircraft.group.updateMatrixWorld(true);
  const inverse=aircraft.group.matrixWorld.clone().invert();let maxMatrixError=0;
  for(const part of BAY_POSES_R14.parts){
  ...
  window.__B24_BAY_R14__={parts:BAY_POSES_R14.parts.length,maxMatrixError,sourcePackage:BAY_POSES.sourcePackage,sourceSHA256:BAY_POSES.sourceSHA256,sideSillFit:BAY_POSES_R14.groups,staticPoseOnly:true};
}
...
function buildInsignia(){
  ...
  $('#decalState').textContent='四处军徽位置保持本轮前确认结果；R14 未调整锚点、尺寸或方向。';
  window.__B24_GENERIC_R14__={...skinSystem.audit,results,closedDoorNodes:[764,767,773,776],split:0,colors:{upper:'#5e5839',lower:'#8b8c97'}};
  let wingIsolationRestore,propVisibilityRestore;
  window.__B24_R14_QA__={
...
$('#details').addEventListener('click',e=>{const b=e.target.closest('[data-detail]');if(!b||!aircraft)return;const v=detailViews[b.dataset.detail];window.__B24_R14_QA__.focus(v.position,v.target);grid.visible=false;status.textContent=v.label;window.__B24_DETAIL_VIEW__=b.dataset.detail;document.querySelectorAll('[data-view]').forEach(el=>el.classList.remove('active'));document.querySelectorAll('[data-detail]').forEach(el=>el.classList.toggle('active',el===b));});
function render(){if(document.hidden)return;orbitControls.update();fixedControls.update();jointDemo?.update(activeCamera);if(jointDemo&&!$('#skinToggle').checked)jointDemo.group.visible=false;renderer.render(scene,activeCamera);}renderer.setAnimationLoop(render);
addEventListener('pagehide',e=>{if(e.persisted)return;renderer.setAnimationLoop(null);jointDemo?.dispose();orbitControls.dispose();fixedControls.dispose();renderer.dispose();});
