...
import {isDetailSurface} from './b24-r9-detail-materials.js';
...
export function createSkinSystem(aircraft,renderer){
  ...
  const shaderCode=`
    ...
    if(engineRadius<.94 && abs(p.x)>2.4 && abs(p.x)<8.8 && p.z>-1.45 && p.z<3.12){
      ...
      float oliveLip=smoothstep(2.70,2.94,p.z);
    ...
    if(engineRadius>.48 && engineRadius<.94 && p.z>-.2 && p.z<2.96 && abs(p.x)>2.4){
      vec3 radial=vec3(p.x-sign(p.x)*engineX,p.y-engineY,0.0);
      if(dot(cameraPosition-p,radial)<0.0)surfacePaint=mix(skinLower*.55,skinUpper,smoothstep(2.68,2.90,p.z));
    }
    // Small source hub caps share a broad shell mesh with unrelated parts.
    if(engineRadius<.18 && p.z>3.1 && p.z<3.7 && abs(p.x)>2.4)surfacePaint=vec3(.25,.28,.29);
  ...
  for(const mesh of paintMeshes){
    ...
    mat.customProgramCacheKey=()=> 'b24-r9-structural-paint-v1';mat.needsUpdate=true;
  }
  const placements=[];
  ...
  return {paintMeshes,placements,meshNode,bodySectionAt,bodySections,wingTriangles,
    ...
    audit:{version:'R9',visualAcceptance:false,productionReady:false,referenceJPEGIdentityVerified:false,
      ...
      nacelleAxes:[{x:3.378,y:-.05645},{x:-3.378,y:-.05645},{x:7.802,y:.09068},{x:-7.802,y:.09068}],
