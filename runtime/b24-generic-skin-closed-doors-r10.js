...
import {BAY_POSES_R8 as BAY_POSES_R10} from './b24-r8-bay-poses.js';
import {applyDetailMaterials} from './b24-r10-detail-materials.js';
import {createSkinSystem,makeInsigniaTexture,makeDecal,makeReferenceSeams} from './b24-r10-skin-system.js';
...
function closeRepairedBombBay(){
  for(const part of BAY_POSES_R10.parts)applyAssetMatrix(part.sourceNode,part.closed);
  aircraft.group.updateMatrixWorld(true);
  const inverse=aircraft.group.matrixWorld.clone().invert();let maxMatrixError=0;
  for(const part of BAY_POSES_R10.parts){
  ...
  window.__B24_BAY_R10__={parts:BAY_POSES_R10.parts.length,maxMatrixError,sourcePackage:BAY_POSES.sourcePackage,sourceSHA256:BAY_POSES.sourceSHA256,sideSillFit:BAY_POSES_R10.groups,staticPoseOnly:true};
}
...
function buildInsignia(){
  ...
  $('#decalState').textContent='四处军徽位置保持本轮前确认结果；R10 未调整锚点、尺寸或方向。';
