...
float dirt=slUse*(1.0-slCare)*(.17*sregion+.38*soot+.20*splash+.1*seam);
...
export function createServiceLife(aircraft,scene,skinSystem,renderer,qa){
  ...
  for(const mesh of skinSystem.paintMeshes){
    ...
    material.onBeforeCompile=shader=>{
      previous(shader);Object.assign(shader.uniforms,uniforms);shader.fragmentShader=helpers+shader.fragmentShader;
      ...
      `);
    };material.customProgramCacheKey=()=> 'b24-r15-service-skin-v1';material.needsUpdate=true;
  }
  ...
  for(const id of [598,681,1200]){
    ...
    m.onBeforeCompile=shader=>{
      ...
      `);
    };m.customProgramCacheKey=()=> 'b24-r15-tire-v1';m.needsUpdate=true;tires.push(id);
  }
  for(const id of [601,684,1203]){
    ...
    `);};original.customProgramCacheKey=()=> 'b24-r15-rim-v1';original.needsUpdate=true;
  ...
  const views={belly:[[.8,-5,5.4],[0,-2,3.3]],port:[[4,-2,3],[.95,-1.4,.15]],starboard:[[-4,-2,3],[-.95,-1.4,.15]],tire:[[5.6,-1.6,2.5],[3.98,-2.15,-.35]],nose:[[1.9,-2.5,5.8],[0,-2.4,4.6]],skin:[[3.2,-.05,-6.9],[.9,-.2,-7.2]],wing:[[7,3.2,3],[6,.3,.5]],exhaust:[[5.7,-2,2],[3.38,-.5,.1]]};
  document.querySelectorAll('[data-service-view]').forEach(b=>b.onclick=()=>{const v=views[b.dataset.serviceView];qa.focus(v[0],v[1]);$('#status').textContent='R15 · '+b.textContent+' · '+state.label;});
  apply();
  window.__B24_SERVICE_R15__={version:'R15',state:()=>({...state,seams:uniforms.slSeams.value}),stats:()=>({drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,geometries:renderer.info.memory.geometries,textures:renderer.info.memory.textures,hardwareVisible:extra.visible,hardwareBounds:extra.children.map(m=>{const b=new THREE.Box3().setFromObject(m);return {min:b.min.toArray(),max:b.max.toArray()};})}),audit:()=>({tires,rims,bolts,paintDatumCorrections:skinSystem.paintDatumCorrections,sourceUVUsed:false,depthBias:{node:1747,factor:1,units:2},surfaceLayout:'illustrative; new procedural domains, no retired seam inputs',paintNodes:skinSystem.paintMeshes.map(m=>m.userData.sourceNode)}),presets:SERVICE_PRESETS};
  return {update:camera=>{extra.visible=camera.position.length()<35;},dispose:()=>disposeGroup(extra)};
}
