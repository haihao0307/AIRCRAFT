...
export function createSkinSystem(aircraft,renderer){
  ...
  // The root has an upper skin without its matching lower skin. Fit the paint
  // datum from the complete thin airfoil on either side, not that lone surface.
  const tailMids=[];
  for(let z=0;z<tailField.height;z++){
    const mids=[];for(let x=0;x<tailField.width;x++){
      const span=-3.45+6.9*x/(tailField.width-1),k=(z*tailField.width+x)*4,d=tailField.data;
      if(Math.abs(span)>1.15&&Math.abs(span)<3.15&&d[k+2]&&d[k+1]-d[k]>.002&&d[k+1]-d[k]<.45)mids.push((d[k]+d[k+1])*.5);
    }mids.sort((a,b)=>a-b);tailMids.push(mids.length?mids[Math.floor(mids.length/2)]:null);
  }
  for(let z=0;z<tailField.height;z++){
    let mid=tailMids[z];for(let d=1;mid===null&&d<tailField.height;d++)mid=tailMids[z-d]??tailMids[z+d]??null;
    if(mid===null)throw new Error('No paired horizontal tail skin');
    for(let x=0;x<tailField.width;x++)tailField.data.set([mid,mid,1,0],(z*tailField.width+x)*4);
  }
  ...
  const shaderCode=`
    ...
    if(nacellePaint>.5 && engineRadius>.48 && engineRadius<.94 && engineZ>2.60 && engineZ<2.96 && abs(p.x)>2.4){
  ...
  for(const mesh of paintMeshes){
    const mat=mesh.material;if(mesh.userData.sourceNode===1747){mat.polygonOffset=true;mat.polygonOffsetFactor=1;mat.polygonOffsetUnits=2;}if([1693,1711].includes(mesh.userData.sourceNode)){mat.polygonOffset=true;mat.polygonOffsetFactor=-1;mat.polygonOffsetUnits=-2;}mat.color.set(0xffffff);mat.metalness=.02;mat.roughness=.76;
    ...
    mat.customProgramCacheKey=()=> 'b24-r16-stable-paint-v1';mat.needsUpdate=true;
  }
  const placements=[];
