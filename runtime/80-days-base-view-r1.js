...
function resize(){const w=stage.clientWidth,h=stage.clientHeight,aspect=w/h;renderer.setSize(w,h,false);perspective.aspect=aspect;perspective.fov=THREE.MathUtils.radToDeg(2*Math.atan(Math.tan(THREE.MathUtils.degToRad(19))*Math.max(1,1.26/aspect)));perspective.updateProjectionMatrix();const hh=Math.max(ortho.userData.halfHeight||15.5,(ortho.userData.minHalfWidth||18.4)/aspect);ortho.left=-hh*aspect;ortho.right=hh*aspect;ortho.top=hh;ortho.bottom=-hh;ortho.updateProjectionMatrix()}
...
export const baseView={THREE,scene,aircraft,skinSystem,renderer,orbitControls,fixedControls,perspective,ortho,setView,setOrtho,resize,getCamera:()=>activeCamera};
