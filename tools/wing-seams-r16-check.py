"""Browser acceptance of R16; --commit tests the immutable public URL."""
...
root=Path(__file__).resolve().parents[1];out=root/'reports/service-life-r16'/('public' if args.commit else 'local');out.mkdir(parents=True,exist_ok=True)
...
report={'url':base+'b24-wing-seams-r16.html','errors':[],'failedResources':[],'states':{},'renderer':'Chrome SwiftShader; not physical mobile performance'}
with sync_playwright() as p:
 ...
 page.wait_for_function('window.__B24_SERVICE_R16__',timeout=120000)
 page.screenshot(path=str(out/'overview.png'))
 report['audit']=page.evaluate('__B24_SERVICE_R16__.audit()');report['source']=page.evaluate('__B24_R16_QA__.sourceBufferAudit()')
 ...
 for view in ['port','starboard','belly','tire','nose','skin','wing','wing-bottom','tail-root','gear-port','gear-starboard','exhaust','exhaust-starboard']:
  page.locator('[data-service-view='+view+']').click();page.wait_for_timeout(150);page.screenshot(path=str(out/(view+'.png')))
 report['stats']=page.evaluate('__B24_SERVICE_R16__.stats()');report['panels']=page.evaluate('__B24_SERVICE_R16__.panels');assert all(v['lookupMisses']==0 for v in report['panels']['regions'].values());assert report['panels']['numericTextureBytes']<262144;print('STATS',report['stats'],flush=True)
 for state in ['new','maintained','field','damage','repaired']:
  page.locator('[data-life='+state+']').click();report['states'][state]=page.evaluate('__B24_SERVICE_R16__.state()');assert report['states'][state]['preset']==state
  for view in ['exhaust','exhaust-starboard']:
 ...
 s=page.evaluate('__B24_SERVICE_R16__.state()');assert s['use']==.82 and s['care']==1 and s['preset']=='custom'
 page.locator('#fullSeams').uncheck();assert page.evaluate('__B24_SERVICE_R16__.state().seams')==0
 ...
 for i,x in enumerate([-1.6,-.8,0,.8,1.6]):
  page.evaluate('v=>__B24_R16_QA__.focus(v,[0,-2,3.3])',[x,-5,5.4]);page.wait_for_timeout(120)
 ...
 # Presentation and measurement use separate camera contracts.
 report['cameras']={}
 for view in ['orbit','perspective-front','perspective-rear','port','starboard','top','bottom','front','rear']:
  page.locator('#views [data-view='+view+']').click();page.wait_for_timeout(150);c=page.evaluate('__B24_R16_QA__.camera()');report['cameras'][view]=c
  expected='perspective' if view in ['orbit','perspective-front','perspective-rear'] else 'orthographic'
  assert c['type']==expected and c['rootScale']==[1,1,1]
  assert c['projection'][11]==(-1 if expected=='perspective' else 0)
  page.screenshot(path=str(out/('camera-'+view+'.png')))
 page.locator('#views [data-view=orbit]').click();page.set_viewport_size({'width':1024,'height':768});page.wait_for_timeout(150)
 assert page.evaluate('__B24_R16_QA__.camera().type')=='perspective'
 page.screenshot(path=str(out/'resized-perspective.png'))
 page.set_viewport_size({'width':390,'height':844});page.wait_for_timeout(150)
 assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
 page.screenshot(path=str(out/'mobile.png'),full_page=True)
 page.locator('#mobileToggle').click();page.locator('[data-life=new]').click();assert page.evaluate('__B24_SERVICE_R16__.state().preset')=='new'
 assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
 ...
 if args.commit:
  report['publicFiles']=[]
  for path in ['b24-wing-seams-r16.html','runtime/b24-service-life-r16.js','runtime/b24-wing-seams-view-r16.js','runtime/b24-r16-skin-system.js','runtime/b24-reference-panels-r16.js','runtime/vendor/three.module.js']:
...
report['passed']=True;(out/'qa.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf8');print('PASS R16',report['url'],flush=True)
