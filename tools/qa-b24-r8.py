"""Browser evidence for the R8 skin candidate. Reference error is separate from technical checks."""
...
ap=argparse.ArgumentParser();ap.add_argument('--url',default='http://127.0.0.1:18767/b24-generic-skin-closed-doors-r8.html');ap.add_argument('--output',type=Path,default=ROOT/'reports/b24-r8');ap.add_argument('--channel',default='chrome');args=ap.parse_args()
args.output.mkdir(parents=True,exist_ok=True)
report={'version':'R8','url':args.url,'commit':subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD'],text=True).strip(),
 'workingTreeDirty':bool(subprocess.check_output(['git','-C',str(ROOT),'status','--porcelain'],text=True).strip()),
 'sourceFiles':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [ROOT/'runtime/b24-r8-skin-system.js',ROOT/'runtime/b24-r7-reference.js',ROOT/'runtime/b24-r8-detail-materials.js',ROOT/'runtime/b24-r8-bay-poses.js',ROOT/'runtime/b24-generic-skin-closed-doors-r8.js',ROOT/'b24-generic-skin-closed-doors-r8.html']},
...
with sync_playwright() as p:
    ...
    try:page.wait_for_function('window.__B24_GENERIC_R8__?.results?.length===4',polling=1000,timeout=120000)
    except Exception:
    ...
    print('Loaded R8; checking source and static coverage',flush=True)
    state=page.evaluate('window.__B24_GENERIC_R8__');report['state']=state
    bay=page.evaluate('window.__B24_BAY_R8__');report['bombBay']=bay
    check('All 60 R8 static fitted bay matrices applied exactly once',bay['parts']==60 and bay['maxMatrixError']<1e-6)
    buffers=page.evaluate('window.__B24_R8_QA__.sourceBufferAudit()');report['sourceBuffers']=buffers
    ...
    details=page.evaluate('window.__B24_DETAILS_R8__');report['details']=details
    ...
    inventory=page.evaluate('window.__B24_R8_QA__.inventory()')
    ...
    check('Visible main and nose tires remain rubber',all(m['color']=='202221' for m in inventory if m['id'] in [598,1200,681]))
    previous=json.loads((ROOT/'reports/b24-r7/qa.json').read_text(encoding='utf8'))['state']
    check('Four accepted mark placements exactly equal R7',state['placements']==previous['placements'])
    check('Mark source-surface samples exactly equal R7',[r['samplePoints'] for r in state['results']]==[r['samplePoints'] for r in previous['results']])
    coverage=page.evaluate('''()=>{
      const ray=window.__B24_R8_QA__.ray,sideMisses=[],bottomMisses=[];let sideRays=0,bottomRays=0;
    ...
    for view in ['orbit','top','bottom','port','starboard','front']:
        ...
        projected=page.evaluate('points=>Object.fromEntries(Object.entries(points).map(([k,p])=>[k,p?window.__B24_R8_QA__.project(p):null]))',samples)
    ...
    for detail in ['bay-port','bay-starboard','bay-bottom','engine','gear','propeller']:
