...
root=Path(__file__).resolve().parents[1];out=root/'reports/rivet-study-r12'/('public' if args.commit else 'local');out.mkdir(parents=True,exist_ok=True)
url='http://127.0.0.1:18768/b24-skin-rivet-study-r12.html';launch=['--enable-webgl','--ignore-gpu-blocklist','--use-angle=swiftshader','--enable-unsafe-swiftshader']
if args.commit:
 ...
 launch+=['--host-resolver-rules=MAP rawcdn.githack.com '+edge];url='https://rawcdn.githack.com/haihao0307/AIRCRAFT/'+args.commit+'/b24-skin-rivet-study-r12.html'
...
with sync_playwright() as p:
 ...
 page.wait_for_function('window.__RIVET_R12__',timeout=120000);page.wait_for_timeout(900)
 for v in ['overview','front','back','section','explode']:
  page.locator('[data-view='+v+']').click();page.wait_for_timeout(350);report['views'][v]=page.evaluate('window.__RIVET_R12__.getState()');page.screenshot(path=str(out/(v+'.png')))
 ...
 page.locator('#separation').fill('3.5');page.locator('#separation').dispatch_event('input');assert page.evaluate('window.__RIVET_R12__.getState().separation')==3.5
 ...
 report['audit']=page.evaluate('window.__RIVET_R12__.audit()')
 ...
 if args.commit:
  report['publicFiles']=[]
  for path in ['b24-skin-rivet-study-r12.html','runtime/rivet-study-r12.js','runtime/vendor/three.module.js','runtime/vendor/three.core.js','runtime/vendor/OrbitControls.js']:
