"""Browser evidence for the R10 skin candidate. Reference error is separate from technical checks."""
...
ap=argparse.ArgumentParser();ap.add_argument('--url',default='http://127.0.0.1:18767/b24-generic-skin-closed-doors-r10.html');ap.add_argument('--output',type=Path,default=ROOT/'reports/b24-r10');ap.add_argument('--channel',default='chrome');args=ap.parse_args()
args.output.mkdir(parents=True,exist_ok=True)
report={'version':'R10','url':args.url,'commit':subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD'],text=True).strip(),
 'workingTreeDirty':bool(subprocess.check_output(['git','-C',str(ROOT),'status','--porcelain'],text=True).strip()),
 'sourceFiles':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [ROOT/'runtime/b24-r10-skin-system.js',ROOT/'runtime/b24-r7-reference.js',ROOT/'runtime/b24-r10-detail-materials.js',ROOT/'runtime/b24-r10-detail-regions.js',ROOT/'runtime/b24-r8-bay-poses.js',ROOT/'runtime/b24-generic-skin-closed-doors-r10.js',ROOT/'b24-generic-skin-closed-doors-r10.html']},
...
with sync_playwright() as p:
    ...
    try:page.wait_for_function('window.__B24_GENERIC_R10__?.results?.length===4',polling=1000,timeout=120000)
    except Exception:
    ...
    print('Loaded R10; checking source and static coverage',flush=True)
    state=page.evaluate('window.__B24_GENERIC_R10__');report['state']=state
    bay=page.evaluate('window.__B24_BAY_R10__');report['bombBay']=bay
    check('All 60 R10 static fitted bay matrices applied exactly once',bay['parts']==60 and bay['maxMatrixError']<1e-6)
    buffers=page.evaluate('window.__B24_R10_QA__.sourceBufferAudit()');report['sourceBuffers']=buffers
    ...
    details=page.evaluate('window.__B24_DETAILS_R10__');report['details']=details
    ...
    inventory=page.evaluate('window.__B24_R10_QA__.inventory()')
    ...
    coverage=page.evaluate('''()=>{
      const ray=window.__B24_R10_QA__.ray,sideMisses=[],bottomMisses=[];let sideRays=0,bottomRays=0;
    ...
    for view in ['orbit','top','bottom','port','starboard','front']:
        ...
        projected=page.evaluate('points=>Object.fromEntries(Object.entries(points).map(([k,p])=>[k,p?window.__B24_R10_QA__.project(p):null]))',samples)
    ...
    role_audit=page.evaluate('window.__B24_R10_QA__.engineRoleAudit()');report['engineRoleAudit']=role_audit
    ...
    page.evaluate('window.__B24_R10_QA__.isolateWings(true)')
    samples=page.evaluate('window.__B24_R10_QA__.wingSurfaceSamples()');report['wingPixelAudit']={}
    for view,side in [('bottom','lower'),('top','upper')]:
        page.locator(f'button[data-view={view}]').click();page.wait_for_timeout(650)
        points=page.evaluate('(a)=>a.map(p=>window.__B24_R10_QA__.project(p))',[a[side] for a in samples])
    ...
    for lip in page.evaluate('window.__B24_R10_QA__.cowlLipSamples()'):
        page.evaluate('(a)=>window.__B24_R10_QA__.focus(...a)',[[lip['x'],lip['y'],6],[lip['x'],lip['y'],2.7]])
        page.wait_for_timeout(650);points=page.evaluate('(a)=>a.map(p=>window.__B24_R10_QA__.project(p))',lip['points']);shot=pixels()
    ...
    page.evaluate('window.__B24_R10_QA__.hidePropellersForAudit(false)')
