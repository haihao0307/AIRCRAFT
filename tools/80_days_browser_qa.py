...
def main():
    ...
    try:
      with sync_playwright() as pw:
        ...
        check('diagnostic-derived-model-hash',state['modelSha256']=='9f1b4efd5da75efee1dddbec1d06b1ad00dfb78dfb8c957a1faecaff23edad1a',state['modelSha256']);check('model-bytes',state['modelBytes']==23789148,state['modelBytes']);check('mesh-inventory',state['meshCount']==348,state['meshCount']);check('liveryuv-candidates',state['candidateCount']==8,state['candidateCount']);check('uv-semantic',state['uvSemantic']=='TEXCOORD_1',state['uvSemantic']);check('uv-layout-explicitly-failed',state['uvLayoutStatus']=='failed',state['uvLayoutStatus']);check('final-liveryuv-not-approved',state['finalLiveryUVApproved'] is False,state['finalLiveryUVApproved']);check('visual-acceptance-blocked',state['visualAcceptance']=='blocked',state['visualAcceptance']);check('no-floating-decals',state['floatingDecals']==0,state['floatingDecals']);check('stam-port-scene-zero',state['stam']['portSceneObjects']==0,state['stam']);check('stam-port-mask-zero',state['stam']['portMasks']==0,state['stam']);check('stam-port-material-zero',state['stam']['portMaterials']==0,state['stam']);check('stam-starboard-one',state['stam']['starboardCount']==1,state['stam']);check('victory-flags-sided',state['victoryFlags']=={'port':0,'starboard':8},state['victoryFlags']);check('bomb-count-blocked',state['bombCount'] is None and state['bombStatus']=='blocked-count' and state['finalBakeApproved'] is False,state)
    ...
    passed=sum(x['ok'] for x in checks);payload={'schema':'haihao.aircraft/80-days-browser-qa@2.1','generatedAtEpoch':time.time(),'testedUrl':url+'80-days-livery-workbench.html','headCommit':None,'sourceModel':{'bytes':23085972,'sha256':'541c3dcfb98ab590cdb1bc90d6ddcdfe80bce2a4b937f3bccefab0c7efe8be0d'},'derivedModel':{'bytes':23789148,'sha256':'9f1b4efd5da75efee1dddbec1d06b1ad00dfb78dfb8c957a1faecaff23edad1a','use':'rejected UV diagnostic only'},'visualAcceptance':'blocked','finalLiveryUVApproved':False,'finalBakeApproved':False,'summary':{'passed':passed,'failed':len(checks)-passed,'total':len(checks)},'checks':checks,'diagnostics':{'consoleErrors':console,'pageErrors':page_errors,'failedRequests':failed},'screenshots':screens};report.write_text(json.dumps(payload,ensure_ascii=False,indent=2));print(json.dumps(payload['summary'],indent=2));return 0 if payload['summary']['failed']==0 else 1
