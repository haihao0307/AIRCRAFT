"""Browser evidence for the R7 skin candidate. Reference error is separate from technical checks."""
...
ap=argparse.ArgumentParser();ap.add_argument('--url',default='http://127.0.0.1:18767/b24-generic-skin-closed-doors-r7.html');ap.add_argument('--output',type=Path,default=ROOT/'reports/b24-r7');ap.add_argument('--channel',default='chrome');args=ap.parse_args()
args.output.mkdir(parents=True,exist_ok=True)
report={'version':'R7','url':args.url,'commit':subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD'],text=True).strip(),
 'workingTreeDirty':bool(subprocess.check_output(['git','-C',str(ROOT),'status','--porcelain'],text=True).strip()),
 'sourceFiles':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [ROOT/'runtime/b24-r7-skin-system.js',ROOT/'runtime/b24-r7-reference.js',ROOT/'runtime/b24-generic-skin-closed-doors-r7.js',ROOT/'b24-generic-skin-closed-doors-r7.html']},
...
with sync_playwright() as p:
    ...
    try:page.wait_for_function('window.__B24_GENERIC_R7__?.results?.length===4',timeout=60000)
    except Exception:
    ...
    state=page.evaluate('window.__B24_GENERIC_R7__');report['state']=state
    bay=page.evaluate('window.__B24_BAY_R7__');report['bombBay']=bay
    check('All 60 supplied repaired bay matrices applied exactly once',bay['parts']==60 and bay['maxMatrixError']<1e-6)
    buffers=page.evaluate('window.__B24_R7_QA__.sourceBufferAudit()');report['sourceBuffers']=buffers
    ...
    check('Paint limited to inherited skin family',state['paintMeshCount']==228)
    ...
    for view in ['orbit','top','bottom','port','starboard','front']:
        ...
        projected=page.evaluate('points=>Object.fromEntries(Object.entries(points).map(([k,p])=>[k,p?window.__B24_R7_QA__.project(p):null]))',samples)
...
print(json.dumps({'passed':report['passed'],'failed':[x for x in checks if not x['passed']],'errors':report['errors'],'placements':state['placements'],'referenceMetrics':report['referenceMetrics']},ensure_ascii=False,indent=2))
