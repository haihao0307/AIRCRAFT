# B-24 Weapons Mother AN/M2 V010

V010 is a single-station corrective evidence gate. It deliberately exposes only the B-24 starboard/right-waist installation for acceptance. Port-waist and A-13 data remain retained for later work but are disabled in this review.
...
- Cases and links use SI-gravity Cannon rigid bodies. They may settle only while touching the real floor or a bounded queue of recent exact settled-debris colliders. Each active piece has a 2–4 impact ceiling, is frozen at its solved transform, and is then removed from the dynamic solver. There is no growing invisible pile box.
